...
describe('pnpm catalog enforcement', () => {
  const packageJson = `{
  "name": "test-package",
  "dependencies": {
    "turndown": "^7.2.2"
...
`;
  ...
  async function lintPackageJson(options: ConfigOptions) {
    const eslint = new ESLint({
      overrideConfig: await defineConfig(options),
    });
    const [result] = await eslint.lintText(packageJson, { filePath: 'package.json' });

    return result?.messages.map((message) => message.ruleId) ?? [];
  }

  it('should not enforce catalogs by default', async () => {
    const ruleIds = await lintPackageJson({});

    expect(ruleIds).not.toContain('pnpm/json-enforce-catalog');
  });

  it('should not enforce catalogs when pnpm is enabled without options', async () => {
    const ruleIds = await lintPackageJson({ pnpm: true });

    expect(ruleIds).not.toContain('pnpm/json-enforce-catalog');
  });

  it('should enforce catalogs when opted in via pnpm.catalogs', async () => {
    const ruleIds = await lintPackageJson({ pnpm: { catalogs: true } });

    expect(ruleIds).toContain('pnpm/json-enforce-catalog');
  });
});
